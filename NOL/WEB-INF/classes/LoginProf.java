...
public class LoginProf extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.println("<body>" + 
                ...
                "   <h1><b>Notas OnLine. </b>Asignaturas de " + profesor.get("nombre").toString() + " " + profesor.get("apellidos").toString() + "</h1>" + 
                "</div>" + 
                "<div class='row' id='CS'><button type='button' onClick='logout()' class='btn btn-link' style='color: red;'>Cerrar sesión</button></div>" + 
                "<div id='descripcion'>" + 
                "   <p><b>En esta página se muestran las asignaturas que impartes</b></p>" + 
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            String nombre = asignatura_json.getString("nombre");

            out.println("<form action='ListaAlumnos' method='GET'>" + 
                    "   <input type='hidden' name='acronimo' value='" + acronimo + "'/>" +
                    "   <ul>" + 
                    "       <li><button type='submit' class='btn btn-link'>" + nombre + "</button></li>" + 
                    "   </ul>" + 
                    "</form>");
